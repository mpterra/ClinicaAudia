...
public class AtendimentoEditDialog extends JDialog {
	...
	public AtendimentoEditDialog(Frame parent, Atendimento atendimento) {
		...
		try {
			...
			// tentar recarregar atendimento completo do banco
			try {
	...
	private void preencherCampos() {
		try {
			...
			lblNomePaciente.setText(
					"Nome: " + (paciente != null && paciente.getNome() != null ? paciente.getNome() : "Não informado"));
			lblTelefone.setText("Telefone: "
					+ (paciente != null && paciente.getTelefone() != null ? paciente.getTelefone() : "Não informado"));
			...
			lblEmail.setText("Email: "
					+ (paciente != null && paciente.getEmail() != null ? paciente.getEmail() : "Não informado"));
		} catch (Exception e) {
	...
	private void atualizarHorarios() {
		...
		try {
			...
			LocalTime proximoDisponivel = null;
			for (int i = 0; i < cbHorario.getItemCount(); i++) {
				LocalTime item = cbHorario.getItemAt(i);
				if (item.equals(horarioOriginal)) {
					cbHorario.setSelectedItem(item);
					cbHorario.setEnabled(true);
					return;
				}
				if (proximoDisponivel == null && item.isAfter(LocalTime.now()))
					proximoDisponivel = item;
			}

			if (proximoDisponivel != null)
				cbHorario.setSelectedItem(proximoDisponivel);
			else if (cbHorario.getItemCount() > 0)
