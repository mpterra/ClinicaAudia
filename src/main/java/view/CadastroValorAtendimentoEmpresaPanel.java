...
// Painel para cadastro e listagem de valores de atendimento por empresa parceira
public class CadastroValorAtendimentoEmpresaPanel extends JPanel {

    private static final long serialVersionUID = 1L;
    private static final Logger LOGGER = Logger.getLogger(CadastroValorAtendimentoEmpresaPanel.class.getName());

    // Componentes de entrada
    private JTextField tfPesquisar, tfPesquisarEmpresa, tfValor;
    private JComboBox<Profissional> cbProfissional;
    private JComboBox<EmpresaParceira> cbEmpresaParceira;
    private JComboBox<ValorAtendimentoEmpresa.Tipo> cbTipoAtendimento;
    ...
    // Construtor
    public CadastroValorAtendimentoEmpresaPanel() {
        setLayout(new BorderLayout(10, 10));
        setBorder(new EmptyBorder(5, 15, 15, 15));
        setBackground(backgroundColor);

        // Título do painel
        JLabel lblTitulo = new JLabel("Cadastro de Valor de Atendimento por Empresa Parceira", SwingConstants.CENTER);
        ...
        btnSalvar.addActionListener(e -> salvarValorAtendimentoEmpresa());

        // Carregar dados iniciais com tratamento de erro
        try {
            carregarProfissionais();
            carregarEmpresasParceiras();
            carregarValoresAtendimentoEmpresa();
        } catch (Exception e) {
    ...
    private JPanel criarPainelCadastro() {
        ...
        panel.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createTitledBorder(
                        ...
                        "Novo Valor de Atendimento por Empresa",
                        TitledBorder.LEFT,
                        TitledBorder.TOP,
                        labelFont,
                        primaryColor),
        ...
        gbc.gridy = 1;
        gbc.weightx = 0.0;
        mainGrid.add(lblEmpresaParceira, gbc);

        cbEmpresaParceira = new JComboBox<>();
        cbEmpresaParceira.setPreferredSize(new Dimension(200, 30));
        cbEmpresaParceira.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        cbEmpresaParceira.setRenderer(new DefaultListCellRenderer() {
            @Override
            public Component getListCellRendererComponent(JList<?> list, Object value, int index,
                                                          boolean isSelected, boolean cellHasFocus) {
                super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
                setText(value instanceof EmpresaParceira ? ((EmpresaParceira) value).getNome() : "");
                return this;
            }
        });
        gbc.gridx = 1;
        gbc.weightx = 1.0;
        mainGrid.add(cbEmpresaParceira, gbc);
        ...
        cbTipoAtendimento = new JComboBox<>(ValorAtendimentoEmpresa.Tipo.values());
    ...
    private JPanel criarPainelTabela() {
        ...
        panel.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createTitledBorder(
                        BorderFactory.createLineBorder(primaryColor, 1, true),
                        "Valores de Atendimento por Empresa Cadastrados",
                        TitledBorder.LEFT,
                        TitledBorder.TOP,
                        labelFont,
                        primaryColor),
        ...
        lblPesquisarProfissional.setFont(labelFont);
        tfPesquisar = new JTextField(15);
        tfPesquisar.setPreferredSize(new Dimension(200, 30));
        panelBusca.add(lblPesquisarProfissional);
        panelBusca.add(tfPesquisar);

        // Pesquisa por Empresa
        JLabel lblPesquisarEmpresa = new JLabel("Pesquisar Empresa:");
        lblPesquisarEmpresa.setFont(labelFont);
        tfPesquisarEmpresa = new JTextField(15);
        tfPesquisarEmpresa.setPreferredSize(new Dimension(200, 30));
        panelBusca.add(lblPesquisarEmpresa);
        panelBusca.add(tfPesquisarEmpresa);

        // Listener para filtragem
        DocumentListener filtroListener = new DocumentListener() {
            ...
            private void filtrarTabela() {
                String textoProfissional = tfPesquisar.getText().trim();
                String textoEmpresa = tfPesquisarEmpresa.getText().trim();

                RowFilter<DefaultTableModel, Integer> filtroProfissional = textoProfissional.isEmpty() ? null : 
                    RowFilter.regexFilter("(?i)" + textoProfissional, 0); // Filtro por profissional (coluna 0)
                RowFilter<DefaultTableModel, Integer> filtroEmpresa = textoEmpresa.isEmpty() ? null : 
                    RowFilter.regexFilter("(?i)" + textoEmpresa, 1); // Filtro por empresa (coluna 1);

                if (filtroProfissional == null && filtroEmpresa == null) {
                    sorter.setRowFilter(null);
                } else if (filtroProfissional != null && filtroEmpresa != null) {
                    sorter.setRowFilter(RowFilter.andFilter(List.of(filtroProfissional, filtroEmpresa)));
                } else {
                    sorter.setRowFilter(filtroProfissional != null ? filtroProfissional : filtroEmpresa);
                }
            }
        };

        tfPesquisar.getDocument().addDocumentListener(filtroListener);
        tfPesquisarEmpresa.getDocument().addDocumentListener(filtroListener);

        // Tabela
        String[] colunas = {"Profissional", "Empresa Parceira", "Tipo", "Valor (R$)"};
    ...
    private void limparCampos() {
        ...
        cbEmpresaParceira.setSelectedIndex(-1);
        cbTipoAtendimento.setSelectedIndex(-1);
    }

    // Salva o valor de atendimento por empresa no banco
    private void salvarValorAtendimentoEmpresa() {
        try {
            ...
            EmpresaParceira empresaSelecionada = (EmpresaParceira) cbEmpresaParceira.getSelectedItem();
            if (empresaSelecionada == null) {
                JOptionPane.showMessageDialog(this, "Selecione uma empresa parceira.", "Erro", JOptionPane.ERROR_MESSAGE);
                return;
            }

            ValorAtendimentoEmpresa.Tipo tipoSelecionado = (ValorAtendimentoEmpresa.Tipo) cbTipoAtendimento.getSelectedItem();
            ...
            // Cria o valor de atendimento por empresa
            ValorAtendimentoEmpresa valorAtendimentoEmpresa = new ValorAtendimentoEmpresa();
            valorAtendimentoEmpresa.setProfissionalId(profissionalSelecionado.getId());
            valorAtendimentoEmpresa.setEmpresaParceiraId(empresaSelecionada.getId());
            valorAtendimentoEmpresa.setTipo(tipoSelecionado);
            valorAtendimentoEmpresa.setValor(valor);
            valorAtendimentoEmpresa.setUsuario(Sessao.getUsuarioLogado().getLogin());

            ValorAtendimentoEmpresaController controller = new ValorAtendimentoEmpresaController();
            boolean sucesso = controller.salvar(valorAtendimentoEmpresa);

            if (sucesso) {
                JOptionPane.showMessageDialog(this, "Valor de atendimento por empresa salvo com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                tfValor.setText("R$ 0,00"); // Reseta apenas o campo de valor
                carregarValoresAtendimentoEmpresa();
            } else {
                JOptionPane.showMessageDialog(this, "Falha ao salvar o valor de atendimento por empresa.", "Erro", JOptionPane.ERROR_MESSAGE);
            }
        } catch (SQLIntegrityConstraintViolationException e) {
            LOGGER.log(Level.SEVERE, "Erro ao salvar valor de atendimento por empresa: " + e.getMessage(), e);
            JOptionPane.showMessageDialog(this, "Já existe um valor de atendimento cadastrado para este profissional, empresa e tipo de atendimento.", "Erro", JOptionPane.ERROR_MESSAGE);
        } catch (SQLException e) {
            LOGGER.log(Level.SEVERE, "Erro ao salvar valor de atendimento por empresa: " + e.getMessage(), e);
            JOptionPane.showMessageDialog(this, "Erro ao salvar valor de atendimento por empresa: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }

    // Carrega os valores de atendimento por empresa na tabela
    private void carregarValoresAtendimentoEmpresa() {
        try {
            ValorAtendimentoEmpresaController controller = new ValorAtendimentoEmpresaController();
            List<ValorAtendimentoEmpresa> valores = controller.listarTodos();
            ...
            for (ValorAtendimentoEmpresa v : valores) {
                String profissionalNome = "Desconhecido";
                String empresaNome = "Desconhecida";
                try {
                    ...
                    EmpresaParceira empresa = empresaController.buscarPorId(v.getEmpresaParceiraId());
                    if (empresa != null) empresaNome = empresa.getNome();
                } catch (SQLException e) {
                    LOGGER.log(Level.WARNING, "Erro ao buscar nomes: " + e.getMessage(), e);
                }
            ...
            LOGGER.log(Level.SEVERE, "Erro ao carregar valores de atendimento por empresa: " + e.getMessage(), e);
            JOptionPane.showMessageDialog(this, "Erro ao carregar valores de atendimento por empresa: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }

    // Carrega os profissionais no JComboBox, ordenados por ID decrescente
    private void carregarProfissionais() {
        ProfissionalController controller = new ProfissionalController();
		List<Profissional> profissionais = controller.listarTodosOrdenadoPorIdDesc();
    ...
    // Carrega as empresas parceiras no JComboBox, ordenadas por ID decrescente
    private void carregarEmpresasParceiras() {
        try {
            EmpresaParceiraController controller = new EmpresaParceiraController();
            List<EmpresaParceira> empresas = controller.listarTodosOrdenadoPorIdDesc();
            cbEmpresaParceira.removeAllItems();
            for (EmpresaParceira empresa : empresas) {
                cbEmpresaParceira.addItem(empresa);
            }
            // Seleciona a primeira empresa, se disponível
            if (cbEmpresaParceira.getItemCount() > 0) {
                cbEmpresaParceira.setSelectedIndex(0);
            }
        } catch (SQLException e) {
            LOGGER.log(Level.SEVERE, "Erro ao carregar empresas parceiras: " + e.getMessage(), e);
            JOptionPane.showMessageDialog(this, "Erro ao carregar empresas parceiras: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
