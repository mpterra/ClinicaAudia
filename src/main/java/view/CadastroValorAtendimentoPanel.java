...
public class CadastroValorAtendimentoPanel extends JPanel {
    ...
    private JPanel criarPainelCadastro() {
        ...
        ValorAtendimento.Tipo[] tiposPermitidos = Arrays.stream(ValorAtendimento.Tipo.values())
                .filter(tipo -> tipo != ValorAtendimento.Tipo.REUNIAO && tipo != ValorAtendimento.Tipo.PESSOAL)
                .toArray(ValorAtendimento.Tipo[]::new);
        cbTipoAtendimento = new JComboBox<>(tiposPermitidos);
    ...
    // Salva o valor de atendimento no banco e cadastra REUNIAO e PESSOAL com R$ 0,00
    private void salvarValorAtendimento() {
        try {
            ...
            // Salva o valor de atendimento selecionado
            ...
            valorAtendimento.setUsuario(usuarioLogado);
            boolean sucesso = controller.salvar(valorAtendimento, usuarioLogado);

            // Cadastra automaticamente REUNIAO e PESSOAL com R$ 0,00, se não existirem
            for (ValorAtendimento.Tipo tipo : new ValorAtendimento.Tipo[]{ValorAtendimento.Tipo.REUNIAO, ValorAtendimento.Tipo.PESSOAL}) {
                try {
                    ValorAtendimento existente = controller.buscarPorProfissionalETipo(profissionalSelecionado.getId(), tipo);
                    if (existente == null) {
                        ValorAtendimento valorAutomatico = new ValorAtendimento();
                        valorAutomatico.setProfissionalId(profissionalSelecionado.getId());
                        valorAutomatico.setTipo(tipo);
                        valorAutomatico.setValor(BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_UP));
                        valorAutomatico.setUsuario(usuarioLogado);
                        controller.salvar(valorAutomatico, usuarioLogado);
                    }
                } catch (SQLException e) {
                    LOGGER.log(Level.WARNING, "Erro ao cadastrar automaticamente " + tipo + " para profissional ID " + profissionalSelecionado.getId() + ": " + e.getMessage(), e);
